...
[点击前往演练场查看 Demo 效果](https://v-md.github.io/v-md/)

## 快速上手
`v-md` 是以 npm 包的形式发布的，你可以将它集成到自己的 Web 应用中。

### 安装依赖
使用自己的包管理工具(`npm/yarn/pnpm`)安装必要的依赖：
```sh
...
const editor = createEditor({
  ...
  // npm 中心仓资源地址。默认为 jsdelivr 的 npm 地址
  cdnUrl: 'https://cdn.jsdelivr.net/npm',
  // npm 中心仓的类型，支持 unpkg 或 jsdelivr。默认为 jsdelivr
  cdnType: 'jsdelivr',
  // 编辑器初始内容。不传则展示初始模板
  value: '',
  // 更多选项 ...
...
### (可选)安装 Vue 语言服务
为了解决 `Web Worker` 在不同构建工具中的兼容性问题，编辑器默认不集成语言服务功能。
...
### 渲染编辑内容
可以通过编辑器实例对象的属性 `editor.files.compiledValue.value` 获取渲染内容。渲染内容是将当前的编译产物代码进行 `gzip` 编码后的字符串。
```ts
import { createEditor } from '@v-md/app'
